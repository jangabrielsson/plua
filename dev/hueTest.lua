...
--%%u:{label="status", text=""}
--%%u:{{button='allLightsOff', text='Lights On'},{button='allLightsOff', text='Lights Off'}}
--%%u:{{button='installLights', text='Install Lights'},{button='removeLights', text='Remove Lights'}}
--%%u:{{button='installSensors', text='Install Sensors'},{button='removeSensors', text='Remove Sensors'}}
--%%u:{{button='installGroups', text='Install Groups'},{button='removeGroups', text='Remove Groups'}}
--%%u:{{button='installCLIPS', text='Install CLIP sensors'},{button='removeCLIPS', text='Remove CLIP sensors'}}
--%%u:{{button='debug', text='Debug On'},{button='list', text="List"}}
--%%u:{{button="connect", text='Authenticate'}}
--%%u:{{label="message", text=""}}
--%%u:{{label="pollingTime", text="Polling time: 1000ms"}}
--%%u:{{slider="pollingSlider", onChanged="polling"}}
--%%u:{{label="pollingFactor", text="Poll lights slower: 1x"}}
--%%u:{{button="b1x", text="1x", onReleased='pollFactor'},{button="b2x", text="2x", onReleased='pollFactor'},{button="b3x", text="3x", onReleased='pollFactor'},{button="b4x", text="4x", onReleased='pollFactor'}}
--%%u:{{button="patch", text='patchQA',onReleased="patchQA"}}
...
function QuickApp:debugf(...) self:debug(string.format(...)) end
...
function QuickApp:onInit()
  ...
  function QuickApp:setView(id,prop,fmt,...) self:updateView(id,prop,fmt,...) end
  ...
  self:setView("pollingTime","text","Polling time: %sms",pollingTime)
  self:setView("pollingFactor","text","Poll lights slower: %sx",pollingFactor) 
