...
python -m src.plua2 --cleanup-port

echo ""
echo "2. Testing cleanup-port with custom API port..."
python -m src.plua2 --api-port 9001 --cleanup-port

echo ""
echo "3. Testing --noapi with inline script (should not start API)..."
python -m src.plua2 --noapi -e 'print("No API server running")' -e 'print("Test completed")'

echo ""
echo "4. Testing custom API port with script..."
echo 'print("Testing custom port")' > /tmp/test_custom_port.lua
python -m src.plua2 --api-port 8891 /tmp/test_custom_port.lua &
TEST_PID=$!
sleep 3
...
python -m src.plua2 --fibaro --api-port 8892 -e 'print("Fibaro:", fibaro ~= nil)' &
FIBARO_PID=$!
sleep 3
